...
uint8 USBMIDI_HandleStandardRqst(void) 
{
    ...
    uint8 bmRequestType = USBMIDI_bmRequestTypeReg;
...
void USBMIDI_ConfigReg(void) 
{
    ...
    USBMIDI_ARB_CFG_REG |= (uint8)  USBMIDI_ARB_CFG_CFG_CMP;
    ...
    USBMIDI_SIE_EP_INT_EN_REG = (uint8) USBMIDI_DEFAULT_SIE_EP_INT_EN;
}
    ...
    if (USBMIDI_configuration > 0u)
    {
        ...
        */
        ...
        for (i = 0u; i < ep; i++)
        {
            ...
            {
                curEp  = ((uint8) pEP->addr & USBMIDI_DIR_UNUSED);
                epType = ((uint8) pEP->attributes & USBMIDI_EP_TYPE_MASK);
            ...
            #if (USBMIDI_EP_MANAGEMENT_DMA_AUTO)
                /* Clear IN data ready. */
                USBMIDI_ARB_EP_BASE.arbEp[curEp].epCfg &= (uint8) ~USBMIDI_ARB_EPX_CFG_IN_DATA_RDY;

                /* Select endpoint number of reconfiguration */
                USBMIDI_DYN_RECONFIG_REG = (uint8) ((curEp - 1u) << USBMIDI_DYN_RECONFIG_EP_SHIFT);
                ...
                if (0u != (pEP->addr & USBMIDI_DIR_IN))
                {
                    ...
                    USBMIDI_EP_TYPE_REG &= (uint8) ~(uint8)((uint8) 0x01u << (curEp - 1u));
                ...
                {
                    /* Set endpoint type: 0 - IN and 1- OUT. */
                    USBMIDI_EP_TYPE_REG |= (uint8) ((uint8) 0x01u << (curEp - 1u));
                ...
                */
                USBMIDI_DYN_RECONFIG_REG &= (uint8) ~USBMIDI_DYN_RECONFIG_ENABLE;
...
uint8 USBMIDI_ClearEndpointHalt(void) 
{
    ...
    if ((ep > USBMIDI_EP0) && (ep < USBMIDI_MAX_EP))
    {
        ...
        USBMIDI_SIE_EP_BASE.sieEp[ep].epCnt0 &= (uint8) ~(uint8)USBMIDI_EPX_CNT_DATA_TOGGLE;
